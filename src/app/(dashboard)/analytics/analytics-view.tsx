    ...
    <div style={{ display: 'grid', gap: '16px' }}>
      <section style={{ ...cardStyle, padding: compact ? '16px' : '20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', marginBottom: '18px' }}>
          <div>
            <h2 style={{ fontSize: compact ? '17px' : '18px', fontWeight: 600, color: '#F0F0F0', margin: 0 }}>
              Operations Pulse
            </h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: chartColumns, gap: '16px' }}>
        <ChartCard title="Activity Trend" subtitle="Administrative activity over the last seven days." badge={`Avg ${data.dailyAverage}/day`} badgeColor="#60A5FA" compact={compact}>
          <LineChart points={data.activityTrend} color="#EA7E20" viewport={viewport} />
        </ChartCard>
        ...
        <ChartCard title="Traffic Trend" subtitle="Recorded session traffic by day from connection history." badge={`Peak ${data.peakTraffic}`} badgeColor="#EA7E20" compact={compact}>
      ...
      <section style={{ ...cardStyle, padding: compact ? '16px' : '20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', marginBottom: '16px' }}>
          <div>
            <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#F0F0F0', margin: 0 }}>Server Traffic</h3>
        ...
        {data.serverTraffic.length === 0 ? (
          ...
          <div style={{ overflowX: 'auto', marginInline: '-4px', paddingInline: '4px' }}>
            <div style={{ border: '1px solid #1E1E1E', borderRadius: '14px', overflow: 'hidden', background: '#0A0A0A', minWidth: '760px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1fr) minmax(90px, 120px) minmax(120px, 150px) minmax(120px, 150px) minmax(220px, 1.2fr)', gap: '16px', padding: '12px 14px', borderBottom: '1px solid #1E1E1E', background: '#101010' }}>
...
function MetricCard({ label, value, accent, helper, compact }: { label: string; value: string; accent: string; helper: string; compact: boolean }) {
  return (
    <div style={{ background: '#0A0A0A', border: '1px solid #1E1E1E', borderRadius: '14px', padding: compact ? '12px' : '14px', minHeight: compact ? '102px' : '108px', display: 'grid', alignContent: 'start' }}>
      ...
      <div style={{ fontSize: '12px', color: '#888888', marginTop: '8px', lineHeight: 1.45 }}>{helper}</div>
    </div>
  )
}
  ...
  return (
    <section style={{ ...cardStyle, padding: compact ? '14px' : '16px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', marginBottom: '14px' }}>
        <div>
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#F0F0F0', margin: 0 }}>{title}</h3>
          <p style={{ fontSize: '12px', color: '#666666', margin: '6px 0 0', lineHeight: 1.5 }}>{subtitle}</p>
        </div>
        {badge ? <span style={badgeStyle(badgeColor ?? '#888888')}>{badge}</span> : null}
      </div>
      {children}
    </section>
  )
  ...
  return (
    <div style={{ display: 'grid', gap: '10px' }}>
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: viewport === 'phone' ? '112px' : '128px', overflow: 'visible' }} preserveAspectRatio="none">
        <line x1={paddingX} y1={height - paddingY} x2={width - paddingX} y2={height - paddingY} stroke="#1E1E1E" strokeWidth="1" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${visiblePoints.length}, minmax(0, 1fr))`, gap: '8px' }}>
        {visiblePoints.map((point) => (
          <div key={point.date} style={{ textAlign: 'center', minWidth: 0 }}>
            <div style={{ fontSize: '11px', fontWeight: 600, color: '#F0F0F0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
  ...
  return (
    <div style={{ display: 'grid', gap: '12px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
        {series.map((item) => (
          <span key={item.label} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#888888', minWidth: 0 }}>
  ...
  return (
    <div style={{ display: 'grid', gap: '14px' }}>
      <svg viewBox="0 0 160 160" style={{ width: `${chartSize}px`, height: `${chartSize}px`, justifySelf: 'center', overflow: 'visible' }} aria-label="Pie chart">
        <circle cx="80" cy="80" r="58" fill="#0A0A0A" stroke="#1E1E1E" strokeWidth="1" />
        {totalValue > 0 ? (
          nonZeroItems.length === 1 ? (
            ...
            nonZeroItems.map((item) => {
              ...
              return <path key={item.label} d={describePieSlice(80, 80, 58, startAngle, endAngle)} fill={item.color} stroke="#111111" strokeWidth="2" />
            })
          )
        ) : null}
        <circle cx="80" cy="80" r="34" fill="#111111" stroke="#1E1E1E" strokeWidth="1" />
        <text x="80" y="74" textAnchor="middle" style={{ fill: '#F0F0F0', fontSize: compact ? '16px' : '18px', fontWeight: 700 }}>{totalValue}</text>
        ...
        {data.map((item) => (
          <div key={item.label} style={{ ...rowCardStyle, padding: compact ? '10px' : '12px', display: 'grid', gap: '6px', minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 600, color: '#F0F0F0', minWidth: 0 }}>
                <span style={{ width: '10px', height: '10px', borderRadius: '9999px', background: item.color, display: 'inline-block', flexShrink: 0 }} />
                {item.label}
              </span>
            ...
            <span style={{ fontSize: '12px', color: '#888888', lineHeight: 1.45 }}>{item.helper}</span>
          </div>
        ))}
      </div>
    </div>
...
function ServerTrafficRow({ server, maxServerTraffic, isLast }: { server: AnalyticsServerTrafficItem; maxServerTraffic: number; isLast: boolean }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1fr) minmax(90px, 120px) minmax(120px, 150px) minmax(120px, 150px) minmax(220px, 1.2fr)', gap: '16px', padding: '14px', borderBottom: isLast ? 'none' : '1px solid #1E1E1E', alignItems: 'center' }}>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: '12px', fontWeight: 600, color: '#F0F0F0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{server.name}</div>
      ...
      <div style={{ fontSize: '12px', fontWeight: 600, color: server.liveSessions > 0 ? '#22C55E' : '#888888' }}>{server.liveSessions}</div>
      <div style={{ fontSize: '12px', fontWeight: 600, color: '#22C55E' }}>{formatBytes(server.bytesIn)}</div>
      <div style={{ fontSize: '12px', fontWeight: 600, color: '#EA7E20' }}>{formatBytes(server.bytesOut)}</div>
      <div style={{ display: 'grid', gap: '6px' }}>
        <div style={{ height: '8px', borderRadius: '9999px', background: '#171717', overflow: 'hidden' }}>
          <div style={{ width: `${Math.max((server.totalTraffic / maxServerTraffic) * 100, server.totalTraffic > 0 ? 6 : 0)}%`, height: '100%', background: 'linear-gradient(90deg, #22C55E 0%, #EA7E20 100%)', borderRadius: '9999px' }} />
        </div>
...
function EmptyState({ text }: { text: string }) {
  return <p style={{ fontSize: '13px', color: '#555555', margin: 0 }}>{text}</p>
}
