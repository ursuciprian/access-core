        ...
        <PageGrid minWidth={420}>
          <section style={{ ...cardStyle, display: 'flex', flexDirection: 'column' }}>
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#F0F0F0', margin: 0 }}>VPN Access</h3>
            ...
            {profile.access.length === 0 ? (
              <p style={{ fontSize: '13px', color: '#888888', margin: 0 }}>No approved VPN access has been granted yet.</p>
            ) : (
                ...
                {profile.access.map((entry) => (
                  <div key={entry.id} style={listCardStyle}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                      <div>
                        <div style={{ fontSize: '14px', fontWeight: 600, color: '#F0F0F0' }}>{entry.server.name}</div>
                        <div style={{ fontSize: '12px', color: '#888888', marginTop: '4px', lineHeight: 1.5 }}>{entry.server.hostname}</div>
                      </div>
